...
export default function StopsSelector() {
  ...
  return (
    <div className="bg-gray-800 rounded-lg shadow-xl p-6">
      ...
      {isOpen && (
        <div className="bg-gray-700 rounded-lg max-h-64 overflow-y-auto mb-4">
          {filteredStops.slice(0, 50).map((stop) => (
            <div
              key={stop.stop_id}
              onClick={() => toggleStop(stop.stop_id)}
              className={`flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-gray-600 border-b border-gray-600 last:border-0 ${
